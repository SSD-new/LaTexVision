...
self.onmessage = async (e) => {
  ...
  if (msg === 'init') {
    waitForOpencv((success) => {
      if (success) {
         self.postMessage({ msg: 'initComplete', success: true });
      } else {
         self.postMessage({ msg: 'error', error: 'OpenCV initialization timed out' });
      }
    });
    return;
  }
  ...
  if (msg === 'segment') {
    if (!self.cv || !self.cv.Mat) {
        self.postMessage({ msg: 'error', error: "OpenCV not ready in worker" });
        return;
    }
    ...
    try {
      ...
      for(let i=0; i<processedBoxes.length; i++) {
          ...
          // --- POST-PROCESSING (Grayscale + Erode) ---
          let processed = new self.cv.Mat();
          self.cv.cvtColor(roi, processed, self.cv.COLOR_RGBA2GRAY);
          let thickenKernel = self.cv.getStructuringElement(self.cv.MORPH_RECT, new self.cv.Size(2, 2));
          self.cv.erode(processed, processed, thickenKernel);
          thickenKernel.delete();
          // --- END POST-PROCESSING ---

          const offscreen = new OffscreenCanvas(finalW, finalH);
          self.cv.imshow(offscreen, processed);
          processed.delete();
          
          const blob = await offscreen.convertToBlob({ type: 'image/png' });
          const dataUrl = await blobToDataURL(blob);

          // IMPORTANT: escaped backticks for template literal inside the worker string
          finalBlocks.push({
